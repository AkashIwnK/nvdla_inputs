...
label_t execute_cv_kernel(label_t in_tr_val, char* found_frame_ptr)
{
  int pid = fork();
  if ( pid == 0 ) { 
        char *arg = {"--loadable", "hpvm-mod_int8.nvdla", "--image", "1_label_7.pgm", "--rawdump" , NULL};
        execvp("nvdla_runtime", arg);
        std::cout << "DONE WITH NVDLA\n";
  } else { 
    int status;
...
// DEBUG(printf("  Calling run_object_detection with in_tr_val tr_val %u %s\n", in_tr_val, object_names[in_tr_val]));
 // Call Keras Code
 label_t objecti;
// = run_object_classification((unsigned)in_tr_val); 
